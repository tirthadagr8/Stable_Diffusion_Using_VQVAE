...
class TimeEmbedding(nn.Module): # just increases the embedding_dim by 4 times (batch,320) -> (batch,1280)
...
class UNET_ResidualBlock(nn.Module):
    ...
    def forward(self, feature, time):
        ...
        feature = feature + time.unsqueeze(-1).unsqueeze(-1)
        
        # (Batch_Size, Out_Channels, Height, Width) -> (Batch_Size, Out_Channels, Height, Width)
        feature = self.groupnorm_merged(feature)
...
class UNET_AttentionBlock(nn.Module):
    def __init__(self, n_head: int, n_embd: int, d_context=768):
        ...
        self.attention_1 = nn.MultiheadAttention(embed_dim=channels,num_heads=n_head)
