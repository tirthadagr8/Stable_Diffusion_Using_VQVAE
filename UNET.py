...
class UNET(nn.Module):
    def __init__(self):
        ...
        for i in range(len(hp.down_blocks)-1):
            self.upblock.append(
                ResidualBlock(hp.up_blocks[i],hp.up_blocks[i+1],hp.temb_dim,hp.num_layers,True,True,hp.text_embed_dim)
            )
            self.upblock.append(nn.ConvTranspose2d(hp.up_blocks[i+1],hp.up_blocks[i+1],kernel_size=4,stride=2,padding=1))
            
        self.norm_out=nn.GroupNorm(hp.norm_channels,hp.up_blocks[-1])
        self.conv_out=nn.Conv2d(hp.up_blocks[-1],3,kernel_size=3,stride=1,padding=1)
